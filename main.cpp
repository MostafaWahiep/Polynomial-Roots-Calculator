...
const double epsilon =
    1e-15;  // stopping criteria when relative less than epsilon

int counter;  // To maintain a count of the number of solutions that have been
              // printed
...
/*
It applies Durand-kerner method, iterating over the initials till it stop have
the solutions
*/

bool CheckValidEquation(string& input_polynomial);
/*
checks the validity of the input string whether it follow the criteria or not to
operate with the equation correctly
*/

vector<pair<double, int> > Operatecoefficients(string& input_polynomial,
                                               int* degree);
/*
returns a vector of pairs; first is the coefficient of order of the second which
is the power
...
int main() {
  ...
  vector<pair<double, int> > poly =
      Operatecoefficients(input_polynomial, &degree);
...
complex<double>* FillIntialGuesses(double coefficients[], int degree) {
  ...
  for (int i = 0; i < degree; i++) {
    complex<double> temp(r * cos(i * theta + theta / 4),
                         r * sin(i * theta + theta / 4));  //
    intial[i] = temp;
  }
...
void iteration(double coefficients[], complex<double>* initial, int degree) {
  ...
  for (; itr < 300; itr++) {
    for (int i = 0; i < degree; i++) {
      ...
      initial[i] =
          initial[i] - SolveFor(initial[i], coefficients, degree) / temp;
    }
  }

  // cout << "It takes "<< itr << " Iterations\n";

  // some solutions would have small imaginary due to floating point imprecision
  // so make them real
  for (int i = 0; i < degree; i++) {
  ...
  // sorting with respect to the absolute of the imaginary part so the real
  // solution are in the begining
  sort(initial, initial + degree,
       [](complex<double> const& a, complex<double> const& b) -> bool {
...
bool CheckValidEquation(string& input_polynomial) {
  int i = 0;
  if (input_polynomial[0] == '-') i++;

  // remove any white spaces
  input_polynomial.erase(
      std::remove_if(input_polynomial.begin(), input_polynomial.end(),
  ...
  for (; i < input_polynomial.size(); i++) {
    ...
    if (!(input_polynomial[i] == 'x' || input_polynomial[i] == '+' ||
          ...
          (input_polynomial[i] >= '0' && input_polynomial[i] <= '9') ||
          input_polynomial[i] == '.'))
      return false;
    // after the power is only a number
    if (input_polynomial[i] == '^' &&
        !(input_polynomial[i + 1] >= '0' && input_polynomial[i + 1] <= '9'))
      return false;
    // after x is space or power or an end line
    if (input_polynomial[i] == 'x' &&
        !(input_polynomial[i + 1] == '+' || input_polynomial[i + 1] == '-' ||
...
vector<pair<double, int> > Operatecoefficients(string& input_polynomial,
                                               int* degree) {
  vector<pair<double, int> > poly;
  int terms = count(input_polynomial.begin(), input_polynomial.end(), '+') +
  ...
  while (terms-- && index < input_polynomial.size()) {
    ...
    if ((input_polynomial[index] >= '0' && input_polynomial[index] <= '9') &&
      ...
      while ((input_polynomial[index + 1] >= '0' &&
             ...
             input_polynomial[index + 1] == '.')  // find the last number
      {
        index++;
      }
